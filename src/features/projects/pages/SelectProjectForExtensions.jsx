...
export default function SelectProjectForExtensions() {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      // ✅ جلب المشاريع التي لديها أمر مباشرة (start_order)
      const { data } = await api.get("projects/?include=start_order,contract");
      let items = Array.isArray(data) ? data : (data?.results || data?.items || []);
      // ✅ فلترة المشاريع التي لديها start_order
      items = items.filter(p => p.start_order_data && p.start_order_data.start_order_date);
      setProjects(items);
    } catch (e) {
  ...
  const handleSelect = () => {
    ...
    // ✅ الانتقال إلى صفحة المشروع مع فتح تبويب أمر المباشرة
    navigate(`/projects/${selectedProjectId}?tab=start_order`);
  };
      ...
      <div className="container" style={{ padding: "var(--space-6)" }}>
        <div style={{ marginBottom: "var(--space-6)" }}>
          <h1 style={{ fontSize: "24px", fontWeight: "600", marginBottom: "8px" }}>
            {t("select_project_for_extensions") || "اختر مشروع لإضافة التمديدات"}
          </h1>
          <p style={{ color: "var(--muted)", fontSize: "14px" }}>
            {t("select_project_for_extensions_desc") || "اختر مشروعاً لديه أمر مباشرة لإضافة التمديدات"}
          </p>
        </div>
        ...
        {projects.length === 0 ? (
          <div className="card" style={{ padding: "var(--space-6)", textAlign: "center" }}>
            <p style={{ color: "var(--muted)", fontSize: "16px" }}>
              {t("no_projects_with_start_order") || "لا توجد مشاريع لديها أمر مباشرة"}
            </p>
          </div>
        ) : (
            ...
            <table className="prj-table" style={{ margin: 0 }}>
              <thead>
                <tr>
                  ...
                  <th>{t("start_order_date") || "تاريخ أمر المباشرة"}</th>
                </tr>
              </thead>
              <tbody>
                {projects.map((p) => (
                  ...
                  >
                    ...
                    <td>
                      {p?.start_order_data?.start_order_date
                        ? formatDate(p.start_order_data.start_order_date, i18n.language)
                        : t("empty_value")}
                    </td>
                  </tr>
                ))}
