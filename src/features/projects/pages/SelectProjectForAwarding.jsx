...
export default function SelectProjectForAwarding() {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      // ✅ جلب المشاريع مع تصنيف القرض السكني فقط
      const { data } = await api.get("projects/?include=contract");
      let items = Array.isArray(data) ? data : (data?.results || data?.items || []);
      
      // ✅ تصفية المشاريع: فقط Housing Loan Program
      items = items.filter((p) => {
        const contractClassification = p?.contract_data?.contract_classification || p?.contract_classification;
        return contractClassification === "housing_loan_program";
      });
  ...
  const handleSelect = () => {
    ...
    // ✅ الانتقال إلى صفحة إضافة الترسية
    navigate(`/projects/${selectedProjectId}/awarding/view`);
  };
      ...
      <div className="container" style={{ padding: "var(--space-6)" }}>
        <div style={{ marginBottom: "var(--space-6)" }}>
          <h1 style={{ fontSize: "24px", fontWeight: "600", marginBottom: "8px" }}>
            {t("select_project_for_awarding") || "اختر مشروع لإضافة ترسية"}
          </h1>
          <p style={{ color: "var(--muted)", fontSize: "14px" }}>
            {t("select_project_for_awarding_desc") || "اختر مشروعاً من نوع برنامج القرض السكني لإضافة ترسية"}
          </p>
        </div>
        ...
        {projects.length === 0 ? (
          <div className="card" style={{ padding: "var(--space-6)", textAlign: "center" }}>
            <p style={{ color: "var(--muted)", fontSize: "16px" }}>
              {t("no_housing_loan_projects") || "لا توجد مشاريع من نوع برنامج القرض السكني"}
            </p>
          </div>
        ) : (
